...
class Settings{
  ...
  constructor({colors = [], width = 1200, height = 600, points = 10, lines = 200} = {}){
    this.width = width;
    this.height = height;
    this.points = points;
    this.lines = lines;
    ...
    createGradientFromColor(color, {stops = 4, colorDelta = 50} = {}) {
      if (!color) return;
      let offsetDelta = Math.floor(100 / stops);
      let offset = 0;
      let parsedColor = color.includes('#') ? "0x" + color.slice(1) : color;
      let colorValue = parseInt(parsedColor, 16);
      this.colors = [];
      while (offset <= 100) {
        let colorValueString = colorValue.toString(16);
        let resultColorString = colorValueString.length < 6? colorValueString.padStart(6, "0") : colorValueString;
        this.colors.push(new GradientStop(offset, "#" + resultColorString));
        colorValue = (colorValue % 0xffffff) + colorDelta;
        offset += offsetDelta;
      }
    }
